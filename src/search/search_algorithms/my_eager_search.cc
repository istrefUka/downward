...
namespace my_eager_search {
My_EagerSearch::My_EagerSearch(
...
void My_EagerSearch::initialize() {
...
void My_EagerSearch::print_statistics() const {
...
void My_EagerSearch::collect_preferred_operators_for_node(
...
void My_EagerSearch::reward_progress() {
    // Boost the "preferred operator" open lists somewhat whenever
    // one of the heuristics finds a state with a new best h value.
    open_list->boost_preferred();
}

void My_EagerSearch::dump_search_space() const {
    search_space.dump(task_proxy);
}

void My_EagerSearch::start_f_value_statistics(EvaluationContext &eval_context) {
    if (f_evaluator) {
...
/* TODO: HACK! This is very inefficient for simply looking up an h value.
   ...
   state. */
void My_EagerSearch::update_f_value_statistics(EvaluationContext &eval_context) {
    if (f_evaluator) {
        int f_value = eval_context.get_evaluator_value(f_evaluator.get());
        statistics.report_f_value_progress(f_value);
    }
